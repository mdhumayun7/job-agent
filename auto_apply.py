...
from config import APPLY_LOG_FILE, JOBS_FILE, LINKEDIN_COOKIE_FILE, PERSONAL_INFO, RESUME_PATH
...
def apply_linkedin(page, job):
    try:
        ...
        if os.path.exists(LINKEDIN_COOKIE_FILE):
            with open(LINKEDIN_COOKIE_FILE, "r", encoding="utf-8") as cf:
        ...
        for _ in range(10):
            ...
            if phone and MY_INFO["phone"]:
                phone.fill(MY_INFO["phone"])
            city = page.query_selector("input[id*=city]")
            if city and MY_INFO["location"]:
                city.fill(MY_INFO["location"])
...
def apply_indeed(page, job):
    try:
        ...
        if "indeed.com" not in url:
            return False, "Not Indeed"
        page.goto(url, timeout=30000)
        page.wait_for_timeout(3000)
        apply_btn = page.query_selector("button[id*=apply]")
        if not apply_btn:
            apply_btn = page.query_selector(".ia-IndeedApplyButton")
        if not apply_btn:
            return False, "No apply button"
        apply_btn.click()
        page.wait_for_timeout(2000)
        name = page.query_selector("input[name*=name]")
        if name and MY_INFO["name"]:
            name.fill(MY_INFO["name"])
        email = page.query_selector("input[type=email]")
        if email and MY_INFO["email"]:
            email.fill(MY_INFO["email"])
...
def save_apply_log():
    ...
    for row, log in enumerate(applied_log, 2):
        ...
        if log["status"] == "Applied":
            sc.fill = PatternFill("solid", fgColor="86EFAC")
        else:
            sc.fill = PatternFill("solid", fgColor="FCA5A5")
        ws.cell(row=row, column=6, value=log["message"])
        ws.cell(row=row, column=7, value=log["link"])
    for col in ws.columns:
        max_len = max((len(str(c.value or "")) for c in col), default=10)
        ws.column_dimensions[col[0].column_letter].width = min(max_len + 4, 50)
    wb.save(APPLY_LOG_FILE)
    print(f"Apply log saved: {APPLY_LOG_FILE}")
    ...
    with sync_playwright() as p:
        ...
        for i, job in enumerate(approved_jobs, 1):
            ...
            if "linkedin" in source:
                success, msg = apply_linkedin(page, job)
            elif "naukri" in source:
                success, msg = apply_naukri(page, job)
            elif "indeed" in source:
                success, msg = apply_indeed(page, job)
            else:
                success, msg = False, "Manual apply needed"
        ...
        browser.close()
    save_apply_log()
    applied = sum(1 for item in applied_log if item["status"] == "Applied")
    failed = len(applied_log) - applied
    print(f"\nDone! Applied: {applied} | Failed: {failed}")
    print(f"Log: {APPLY_LOG_FILE}")
